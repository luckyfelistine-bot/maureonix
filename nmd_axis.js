...
// ══════════════════════════════════════════════════════════════
// 🕐 Auto Delete Helper — text messages delete after 5.5 minutes
// ══════════════════════════════════════════════════════════════
...
function _secsToEnglish(secs) {
    ...
    const rem  = secs % 60;
    let timeStr;
    if (mins > 0 && rem > 0) timeStr = `${mins} minutes and ${rem} seconds`;
    else if (mins > 0)        timeStr = `${mins} minutes`;
    else                      timeStr = `${rem} seconds`;
    return `⏱️ *This message will self-destruct in ${timeStr}* (${secs}s)`;
}
...
class MusicDownloader {
    ...
    async _tryMethods(methods, input = '', progressCallback = null) {
        ...
        return { success: false, error: 'All methods failed', attempts };
    }
...
module.exports = nmd_axis = async (nimesha, m, msg, store) => {
    try {
        ...
        const botFooter = global.db?.set?.[botNumber]?.botname
            ...
            : global.mess?.footer || '> *Maureonix* [BOT] | CREATED BY INFINITE VYBEFLIX';
        ...
        } else if (isCmd && isTrusted && m.isGroup) {
            const userMsgKey = m.key;
            setTimeout(async () => {
                try { await nimesha.sendMessage(m.chat, { delete: userMsgKey }); } catch(e) {}
            }, AUTO_DELETE_SECS * 1000);
            ...
            if (pending.type === 'video') {
                ...
                try {
                    ...
                    const friendlyErr = errMsg.includes('ffmpeg') ? 'ffmpeg not installed — `pkg install ffmpeg`'
                        : errMsg.includes('yt-dlp') ? 'yt-dlp not installed or needs update'
...
${botFooter}`;
        ...
        // .joke
            ...
            const jokeMsg = await nimesha.sendMessage(m.chat, { text: `😂 *Fetching joke...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
            ...
            await nimesha.sendMessage(m.chat, { text: joke ? `${joke}\n\n━━━━━━━━━━━━━━━━━━━━━━\n${botFooter}` : `❌ Could not fetch a joke.\n${botFooter}`, edit: jokeMsg.key });
        }

        // .quote
        else if (cmd === 'quote') {
            const quoteMsg = await nimesha.sendMessage(m.chat, { text: `💬 *Fetching quote...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
            ...
            await nimesha.sendMessage(m.chat, { text: quote ? `${quote}\n\n━━━━━━━━━━━━━━━━━━━━━━\n${botFooter}` : `❌ Could not fetch a quote.\n${botFooter}`, edit: quoteMsg.key });
        }

        // .fact
        else if (cmd === 'fact') {
            const factMsg = await nimesha.sendMessage(m.chat, { text: `💡 *Fetching fact...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
            ...
            await nimesha.sendMessage(m.chat, { text: fact ? `💡 *Interesting Fact!*\n━━━━━━━━━━━━━━━━━━━━━━\n${fact}\n━━━━━━━━━━━━━━━━━━━━━━\n${botFooter}` : `❌ Could not fetch a fact.\n${botFooter}`, edit: factMsg.key });
        }

        // .define <word>
        else if (cmd === 'define') {
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a word!\nExample: ${prefix}define hello`, botFooter, { quoted: m });
            const defineMsg = await nimesha.sendMessage(m.chat, { text: `📖 *Searching "${q}"...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
        ...
        // .weather <city>
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a city name!\nExample: ${prefix}weather Nairobi`, botFooter, { quoted: m });
            const weatherMsg = await nimesha.sendMessage(m.chat, { text: `🌤️ *Fetching weather...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
        ...
        // .news
            ...
            const newsMsg = await nimesha.sendMessage(m.chat, { text: `📰 *Fetching news...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
            ...
            await nimesha.sendMessage(m.chat, { text: news ? `📰 *Latest News*\n━━━━━━━━━━━━━━━━━━━━━━\n${news}\n━━━━━━━━━━━━━━━━━━━━━━\n${botFooter}` : `❌ Could not fetch news.\n${botFooter}`, edit: newsMsg.key });
        }

        // .lyrics <song>
        else if (cmd === 'lyrics') {
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a song name!\nExample: ${prefix}lyrics Shape of You`, botFooter, { quoted: m });
            const lyricsMsg = await nimesha.sendMessage(m.chat, { text: `🎵 *Searching lyrics...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
        ...
        // .8ball <question>
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a question!\nExample: ${prefix}8ball Will I win?`, botFooter, { quoted: m });
        ...
        // .tts <text>
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter text!\nExample: ${prefix}tts hello world`, botFooter, { quoted: m });
        ...
        // .trt <text> <lang>
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter text and language!\nExample: ${prefix}trt Hello si\nExample: ${prefix}trt Ayubowan en`, botFooter, { quoted: m });
        ...
        // .ss <link>
            ...
            if (!q || !q.match(/https?:\/\//)) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a URL!\nExample: ${prefix}ss https://google.com`, botFooter, { quoted: m });
            ...
            if (imgBuffer) {
                ...
                await nimesha.sendMessage(m.chat, { text: `❌ Failed to take screenshot.\n${botFooter}`, edit: waitMsg.key });
            }
        }
        ...
        // .cinfo <country>
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a country name!\nExample: ${prefix}cinfo Kenya`, botFooter, { quoted: m });
            const cinfoMsg = await nimesha.sendMessage(m.chat, { text: `🌍 *Searching country...*\n⏳ Please wait...\n${botFooter}` }, { quoted: m });
...
━━━━━━━━━━━━━━━━━━━━━━`;
        ...
        // .welcome on/off / .setwelcome / .setleave
            ...
            if (!m.isGroup) return await sendAutoDelete(nimesha, m.chat, `❌ Group command only!`, botFooter, { quoted: m });
            if (!m.isAdmin) return await sendAutoDelete(nimesha, m.chat, `❌ Admin command only!`, botFooter, { quoted: m });
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter leave text!\nExample: ${prefix}setleave @ left the group.`, botFooter, { quoted: m });
            if (!global.db.groups) global.db.groups = {};
            if (!global.db.groups[m.chat]) global.db.groups[m.chat] = {};
            if (!global.db.groups[m.chat].text) global.db.groups[m.chat].text = {};
            global.db.groups[m.chat].text.setleave = q;
            await sendAutoDelete(nimesha, m.chat, `✅ *Custom Leave Message set!*\n━━━━━━━━━━━━━━━━━━━━━━\n📝 *Preview:*\n${q.replace('@', '@' + (m.sender.split('@')[0]))}\n━━━━━━━━━━━━━━━━━━━━━━`, botFooter, { quoted: m });
        }
        ...
        // .imagine / .flux / .sora (AI image)
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a prompt!\nExample: ${prefix}${cmd} a beautiful sunset`, botFooter, { quoted: m });
            const waitMsg = await nimesha.sendMessage(m.chat, { text: `🎨 *Generating AI image...*\n━━━━━━━━━━━━━━━━━━━━━━\n✨ *Prompt:* ${q}\n⏳ Please wait...\n━━━━━━━━━━━━━━━━━━━━━━\n${botFooter}` }, { quoted: m });
            ...
            if (imgBuffer) {
                ...
                await nimesha.sendMessage(m.chat, { text: `❌ Failed to generate image.\n${botFooter}`, edit: waitMsg.key });
            }
        }
        ...
        // .removebg
            ...
            try {
                ...
                if (result) {
                    ...
                    await editAutoDelete(nimesha, m.chat, `✅ *Background removed!*`, botFooter, waitMsg.key);
                } else { await nimesha.sendMessage(m.chat, { text: `❌ Failed to remove background.\n${botFooter}`, edit: waitMsg.key }); }
            } catch (e) { await sendAutoDelete(nimesha, m.chat, `❌ Error: ${e.message}`, botFooter, { quoted: m }); }
        }
        ...
        // .attp (animated text sticker) — ffmpeg directly → webp, API fallback
            ...
            try {
                ...
                await editAutoDelete(nimesha, m.chat, `✅ *ATTP sticker generated!*\n🎨 *Text:* ${q}`, botFooter, atttpWaitMsg.key);
            } catch (ffErr) {
        ...
        // ══════════════════════════════════════════════════════════════
            ...
            } else { await nimesha.sendMessage(m.chat, { text: `❌ Failed to generate text art.\n${botFooter}`, edit: waitMsg.key }); }
        }
        ...
        // ══════════════════════════════════════════════════════════════
            ...
            } else await sendAutoDelete(nimesha, m.chat, `❌ Could not fetch ${cmd} GIF.`, botFooter, { quoted: m });
        }
        ...
        // ══════════════════════════════════════════════════════════════
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter a quote!\nExample: ${prefix}oogway Yesterday is history`, botFooter, { quoted: m });
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter tweet text!\nExample: ${prefix}tweet Hello World!`, botFooter, { quoted: m });
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter comment text!\nExample: ${prefix}ytcomment This video is amazing!`, botFooter, { quoted: m });
            const username = m.pushName || 'User';
        ...
        // ══════════════════════════════════════════════════════════════
            ...
            if (!q) return await sendAutoDelete(nimesha, m.chat, `⚠️ Please enter an app name!\nExample: ${prefix}apk WhatsApp`, botFooter, { quoted: m });
