...
async function handleAIQuery(sock, msg, ctx, query) {
  // PRIVATE MODE: If enabled, only respond in private chats
  if (global.privateMode === true && ctx.isGroup) {
    return; 
  }

  // PUBLIC MODE: If disabled, only owner can use
  if (global.public === false && !isOwner(ctx.sender)) {
    return;
  }
...
registerCommand('private', async (sock, msg, ctx, parsed) => {
  if (!isOwner(ctx.sender)) {
    await safeReply(sock, ctx.remoteJid, '❌ Only the owner can toggle private mode.');
    return;
  }
  ...
  if (typeof global.togglePrivateMode === 'function') {
    ...
    await safeReply(sock, ctx.remoteJid, newState 
      ? '🔒 *Private Mode ON* — Bot will only respond in private chats.' 
      : '🔓 *Private Mode OFF* — Bot responding normally in all chats.'
    );
  } else {
    // Fallback if settings.js hasn't been updated
    global.privateMode = !global.privateMode;
    await safeReply(sock, ctx.remoteJid, global.privateMode 
      ? '🔒 *Private Mode ON* — Bot will only respond in private chats.' 
      : '🔓 *Private Mode OFF* — Bot responding normally in all chats.'
    );
  }
...
registerCommand('public', async (sock, msg, ctx, parsed) => {
  if (!isOwner(ctx.sender)) {
    await safeReply(sock, ctx.remoteJid, '❌ Only the owner can toggle public mode.');
    return;
  }
  
  global.public = !global.public;
  await safeReply(sock, ctx.remoteJid, global.public 
    ? '🔓 *Public Mode ON* — Everyone can use the bot.' 
    : '🔒 *Public Mode OFF* — Only owner can use the bot.'
  );
