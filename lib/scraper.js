...
async function spotifyDownload(url) {
  const r = await fetch(`https://api.spotifydl.xyz/download?url=${encodeURIComponent(url)}`);
  const d = await r.json();
  if (!d.download) throw new Error('No Spotify link');
  return { url: d.download, title: d.title, artist: d.artist };
}
