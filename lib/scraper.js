...
async function getCobaltInstances() {
    try {
        const res = await fetch('https://instances.cobalt.best/api/instances.json', { signal: AbortSignal.timeout(8000) });
        const data = await res.json();
        return (data || [])
...
async function runMethods(methods, label) {
    ...
    throw new Error(`All ${label} methods failed!`);
}
...
async function tiktokDownload(url) {
    ...
    throw new Error('All TikTok methods (21) failed!');
}
...
async function fbDownload(url) {
    ...
    const methods = isShareUrl ? [ytdlpMethod, ...webMethods] : [...webMethods, ytdlpMethod];
...
async function redditDownload(url) {
    const methods = [
        ...
        async () => {
            ...
            const res = await fetch(apiUrl, { headers: { 'User-Agent': 'Maureonix-bot/1.0' }, signal: AbortSignal.timeout(15000) });
            const data = await res.json();
