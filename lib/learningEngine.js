// ═══════════════════════════════════════════════════════════════════════════
//   🦊 MAUREONIX CORTEX LEARNING ENGINE v1.1 — "THE MIND FORGE"
...
class LearningSessionManager {
    ...
    endSession(userId) {
        ...
        return { mastery, totalChunks: session.chunks.length, chunksStudied: session.currentChunkIndex + 1, curriculumName: session.curriculumName };
    }
}
...
class LearningEngine {
    ...
    async processLearningQuery(text, userId) {
        if (!text || !text.trim()) return { type: 'text', text: 'Please provide a command or answer.' };
        const lower = text.toLowerCase().trim();

        // Allow exit and stop commands
        if (['exit', 'quit', 'stop', 'exitlearn'].includes(lower) || lower.startsWith('.exitlearn') || lower.startsWith('.stop') || lower.startsWith('.exit')) {
            const result = this.sessionManager.endSession(userId);
