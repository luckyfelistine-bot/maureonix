...
async function executeActions(responseText, maureonix, ownerId, m, coreHandler = null) {
    const actions = [];
            ...
            switch (action.type) {
               ...
               // ── Run a bot command on behalf of the owner (fixed) ──
                case 'run_bot_command': {
                    try {
                        ...
                        if (typeof coreHandler === 'function') {
                            await coreHandler(maureonix, fakeM, null, null);
                ...
                case 'send_message': {
                    ...
                    await maureonix.sendMessage(target, { text: lines.slice(1).join('\n') });
                ...
                case 'schedule_message': {
                    ...
                    setTimeout(() => { maureonix.sendMessage(d.contact || d.target, { text: d.message || d.text }).catch(() => {}); }, delayMs);
                ...
                case 'run_diagnosis': {
                    ...
                    if (diag.status === 'CRITICAL') await sendCrisisAlert('Self-diagnosis CRITICAL.', ownerId, 'CRITICAL', maureonix);
                    break;
                }
...
async function handleApproval(userMessage, maureonix, m) {
    ...
    if (!filePath.startsWith(PROJECT_ROOT)) {
        await maureonix.sendMessage(m.chat, { text: '❌ Invalid file path.' });
        return true;
    }
...
async function handleOwnerMessage(maureonix, m, ctx) {
    ...
    if (userMessage.toUpperCase().startsWith('APPROVE ')) {
        return handleApproval(userMessage, maureonix, m);
    }
    ...
    if (global.maureonixActivated && /^(sleep|deactivate|lock|goodbye)$/i.test(userMessage)) {
        ...
        await maureonix.sendMessage(m.chat, { text: '💤 *Maureonix Core deactivated.* I\'ll be here when you need me, Creator.' });
        return;
    }
    ...
    if (!global.maureonixActivated) {
        if (userMessage === config.maureonixPassphrase) {
            ...
            await maureonix.sendMessage(m.chat, { text: '🔐 *Access denied.* Provide the passphrase.' });
            return;
        }
    }
    ...
    //   Route complex requests through the cognitive engine
    // ═══════════════════════════════════════════════════════════════════════════════

    let enhancedContext = '';
    let isSuperComplex = false;
    try {
        ...
        isSuperComplex = userMessage.length > 80 || 
                         /design|architecture|plan|strategy|predict|analyze|audit|improve|optimize|create|invent/.test(userMessage.toLowerCase());

        if (isSuperComplex) {
            const cognition = await superIntelligence.think(userMessage, {
                userId,
                timestamp: Date.now(),
                previousMessages: [],
            });
            ...
            if (cognition.memories?.length > 0) {
                cognitiveContext += `\n📚 Relevant memories recalled: ${cognition.memories.length}`;
            }

            if (cognition.creativeIdeas?.length > 0) {
                cognitiveContext += `\n💡 Creative angles:`;
                for (const idea of cognition.creativeIdeas.slice(0, 3)) {
                    cognitiveContext += `\n   • ${idea.idea.slice(0, 120)}... (novelty: ${Math.round(idea.novelty * 100)}%)`;
                }
            }

            if (cognition.swarmConsensus) {
                cognitiveContext += `\n🤖 Swarm consensus: ${cognition.swarmConsensus.winner} agent selected (confidence: ${Math.round(cognition.swarmConsensus.confidence * 100)}%)`;
            }

            if (cognition.newCapability) {
                cognitiveContext += `\n🔧 Auto-generated capability: ${cognition.newCapability.file}`;
            }

            cognitiveContext += `\n🎯 Confidence: ${Math.round(cognition.confidence * 100)}%`;

            enhancedContext = cognitiveContext;

            await superIntelligence.memory.store({
                content: `Owner asked: ${userMessage}. Response planned with confidence ${cognition.confidence}`,
                type: 'owner_interaction',
                domain: 'conversation',
                importance: 0.7,
                context: { userId, cognition: cognition.thoughtId },
            });
        }
    } catch (e) {
        console.log('[SuperIntelligence] Fallback:', e.message);
    }

    if (enhancedContext) {
        global._lastCognitiveContext = enhancedContext;
    }
    
    let finalText = aiResult.text;

    // Append cognitive context if available
    if (global._lastCognitiveContext && isSuperComplex) {
        finalText += global._lastCognitiveContext;
        global._lastCognitiveContext = null;
    }
    
    // ── Pass coreHandler to executeActions ──
    finalText = await executeActions(finalText, maureonix, userId, m, coreHandler);
    
    await AI.sendLongMessage(maureonix, m.chat, `🦊 *Maureonix*\n\n${finalText}`, { quoted: m });
...
async function sendCrisisAlert(userMessage, userId, severity, maureonix) {
    const { sendEmail } = require('./emailService');
    const config = require('../config');
    ...
    if (maureonix && config.ownerNumber?.[0]) {
        try { await maureonix.sendMessage(config.ownerNumber[0] + '@s.whatsapp.net', { text: msg }); }
...
async function handleSymphonyCommand(maureonix, m, command, args) {
