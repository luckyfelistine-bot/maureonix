...
// ✅ DEFAULT DATABASE STRUCTURE – ensures all collections exist
const defaultDB = {
  users: {},
  groups: {},
  chats: {},
...
class MongoDB {
    ...
    read = async () => {
        ...
        try {
            ...
            // ✅ Merge with defaults even if parsing fails
            return { ...defaultDB, ...doc.data };
        }
    };
        ...
        if (fs.existsSync(this.file)) {
            try {
                ...
                if (fs.existsSync(this.file + '.bak')) {
                    ...
                    data = defaultDB;
                    fs.writeFileSync(this.file, JSON.stringify(defaultDB, null, 2));
                }
            }
        } else {
            data = defaultDB;
...
const cmdAdd = (hit) => {
    if (!hit) return;  // ← Guard against undefined
...
const checkExpired = (_dir, conn) => {
    if (!Array.isArray(_dir)) {
        console.error('⚠️ checkExpired called with non-array:', _dir);
        return;
    }
    setInterval(() => {
