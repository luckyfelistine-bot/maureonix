...
async function gifToWebp(media) {
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            .on('error', reject)
            .on('end', () => resolve(true))
            .addOutputOptions(['-vf', 'scale=512:512:force_original_aspect_ratio=decrease', '-loop', '0', '-preset', 'default', '-an', '-vsync', '0'])
            .toFormat('webp')
            .save(tmpFileOut);
    });
...
async function imageToWebp(media) {
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            .on('error', reject)
            .on('end', () => resolve(true))
            .addOutputOptions(['-vcodec', 'libwebp', '-vf', 'scale=500:500:force_original_aspect_ratio=decrease,setsar=1, pad=500:500:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse', '-loop', '0', '-preset', 'default'])
            .toFormat('webp')
            .save(tmpFileOut);
    });
    const buff = fs.readFileSync(tmpFileOut);
...
async function writeExif(media, data) {
    ...
    if (data) {
        ...
        const { wra = data.pack_id ? data.pack_id : global.author ? global.author : 'Maureonix', wrb = data.packname ? data.packname : global.packname ? global.packname : 'Maureonix', wrc = data.author ? data.author : global.author ? global.author : 'Infinite Vybeflix', wrd = data.categories ? data.categories : [''], wre = data.isAvatar ? data.isAvatar : 0, ...wrf } = data;
