...
async function askModel(prompt, modelName, userId) {
    ...
    switch (modelName.toLowerCase()) {
        case 'gpt':
        case 'chatgpt':
            systemHint = 'You are ChatGPT. Answer concisely and directly.';
            preferredModel = MODELS.gpt20b;
            break;
        case 'gemini':
            systemHint = 'You are Gemini. Answer concisely and directly.';
            preferredModel = MODELS.gemma;
            break;
        case 'llama':
        case 'llama3':
            systemHint = 'You are Llama 3. Answer concisely and directly.';
            preferredModel = MODELS.versatile;
            break;
        case 'deepseek':
            systemHint = 'You are DeepSeek. Answer concisely and directly.';
            preferredModel = MODELS.qwen;
            break;
        default:
            systemHint = 'You are Maureonix. Answer concisely and directly.';
            preferredModel = TASK_MODEL.conversation;
    }
    try {
        // Use ultimateAI to ensure we get a clean answer (no thinking in output)
