...
// ═══════════════════════════════════════════════════════════════
async function selfChatAI(userMessage, userId, availableCommands = null) {
    const userMsg = userMessage.trim();
    if (!userMsg) return { type: 'text', text: 'Hello! How can I help?' };

    // ── Tier 1: Rule-based (instant, deterministic) ──
    const ruleIntent = detectIntent(userMsg);
    if (ruleIntent && ruleIntent.confidence === 'high') {
        return {
            type: 'function',
            function: ruleIntent.cmd,
            args: ruleIntent.args,
            confidence: ruleIntent.confidence,
            source: ruleIntent.source
        };
    }

    // ── Tier 2: LLM-based (handles nuance, synonyms, complex requests) ──
    const llmResult = await detectIntentLLM(userMsg, userId);
    if (llmResult) {
        if (llmResult.type === 'text') {
            return { type: 'text', text: llmResult.text, source: 'llm' };
        }
        if (llmResult.cmd) {
            return {
                type: 'function',
                function: llmResult.cmd,
                args: llmResult.args,
                confidence: 'llm',
                source: 'llm'
            };
    ...
    // ── Tier 3: Fallback to conversation ──
    try {
