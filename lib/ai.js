...
const CRISIS_KEYWORDS = [
    ...
    'self harm', 'cut myself', 'hurt myself', 'overdose', 'death', 'dying',
...
function detectCrisis(text) {
    ...
    for (const kw of CRISIS_KEYWORDS) {
        if (lower.includes(kw)) {
            return { isCrisis: true, severity: 'high', matchedKeyword: kw };
        }
    }
    // also check for very strong distress phrases
    if (lower.match(/\b(please help me|i need help|i'm scared|i'm alone|someone talk to me)\b/)) {
        return { isCrisis: true, severity: 'medium', matchedKeyword: 'distress phrase' };
    }
    return { isCrisis: false };
}

/**
 * Handles a crisis detection.
 * @param {string} userId - User's JID
 * @param {string} text - The message that triggered crisis
 * @param {object} sock - nimesha socket
 * @param {object} m - message object (for reply)
 * @param {object} db - global database
 * @param {string|string[]} ownerNumber - Owner's JID(s)
 * @returns {Promise<boolean>} true if handled (crisis), false if not
 */
async function handleCrisis(userId, text, sock, m, db, ownerNumber) {
    // Check if crisis detection is globally enabled
    if (!db.set?.crisisDetection) return false;

    // Avoid spamming: only trigger once per user every 30 minutes
    const lastTrigger = CRISIS_STORAGE.get(userId);
    if (lastTrigger && (Date.now() - lastTrigger.triggeredAt) < 30 * 60 * 1000) {
        return true; // already handled recently, don't repeat
    }
    ...
    // 1. Send a caring, non‑judgmental response to the user
...
/**
 * Process a user's reply (yes/no) to the crisis offer.
 * Call this from your main handler when a user replies with 'yes' or 'no'.
 */
async function processCrisisResponse(userId, response, sock, db, ownerNumber) {
    ...
    delete db.crisisResponses[userId]; // remove pending flag

    const lower = response.trim().toLowerCase();
    if (lower === 'yes') {
        // Send user the owner's contact information
        const ownerJids = Array.isArray(ownerNumber) ? ownerNumber : [ownerNumber];
