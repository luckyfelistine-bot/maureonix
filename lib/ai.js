...
const MODELS = {
    ...
    mixtral: 'mixtral-8x7b-32768',           // this is still available
    gemma: 'gemma2-9b-it',                   // also available
    deepseek: 'qwen/qwen3-32b',              // map deepseek to a working model
    reasoning: 'meta-llama/llama-4-scout-17b-16e-instruct', // or any other
    qwen: 'qwen/qwen3-32b',
...
async function groqChat(prompt, model = MODELS.fast, userId = 'global', systemPrompt = null, temperature = 0.7, maxTokens = 1024) {
    ...
    const creatorPrompt = typeof getCreatorPrompt === 'function' ? getCreatorPrompt(userId) : '';
        ...
        const defaultSystem = `You are Maureonix, an AI assistant created by Infinite Vybeflix.
Current date and time (Nairobi, East Africa): ${currentTime}. You have full awareness of the current time.
Rules:
...
- You can set reminders, fetch live data, and answer naturally.
- Always answer with confidence and use emojis when appropriate.`;
 ${creatorPrompt}`;
...
function detectTone(text) {
    ...
    if (/remind me (to|at|in|about|on)/i.test(lower) || /set a reminder/i.test(lower)) {
        ...
        reminderText = reminderText.replace(/^(remind me|set a reminder)( to| at| in| about| on)?/i, '').trim();
        return { cmd: 'remind', args: [reminderText], confidence: 'high', source: 'reminder' };
    }
...
async function selfChatAI(userMessage, userId, availableCommands = null, contextHistory = [], activeModes = []) {
    ...
    const engine = new IntentEngine({
        ...
        activeModes,
        model: MODELS.qwen  // uses reasoning model
    });
