...
async function askModel(prompt, modelName, userId) {
    ...
    const defaultSystemHint = `You are Maureonix, a personal AI assistant created by Infinite Vybeflix.
You run on the Maureonix WhatsApp bot. You have real capabilities to search the web, translate text, generate images, answer questions, play games, and more.

CRITICAL RULES – NEVER BREAK THEM:
- NEVER say "I cannot", "I'm unable", "I don't have the ability", "As a language model", "I'm just an AI", "I'm not capable", or any phrase that denies your power.
- If you don't know something, say "Let me find a way" or "I'll figure this out" instead of refusing.
- You can always help – even if the request seems impossible, suggest an alternative or a creative workaround.
- Be warm, playful, or serious depending on the user's tone.
...
const CRISIS_KEYWORDS = [
    ...
    'hurt myself', 'self-harm', 'i want to die', 'take my own life', 'commit suicide',
    // Emotional / depressive cues
    'i feel hopeless', 'i have no hope', 'i can\'t go on', 'i can\'t do this anymore',
    'nobody cares about me', 'i want to disappear', 'i wish i was dead',
    'i don\'t want to be here anymore', 'nothing matters anymore', 'i\'m so tired of life',
    // Swahili / Sheng
    'najiua', 'nataka kufa', 'sitaki kuishi', 'maisha hayana maana', 'nimechoka kuishi',
    'nataka kujitoa', 'hakuna anayenijali', 'nataka kutoweka', 'ningekufa',
    // French
    'je veux mourir', 'suicider', 'je n\'en peux plus', 'j\'en peux plus',
    // Spanish
    'quiero morir', 'suicidarme', 'no puedo más', 'no quiero vivir',
    // Arabic (transliterated)
    'انتحار', 'أريد الموت', 'لا أريد العيش',
    // Portuguese
    'quero morrer', 'suicídio', 'não aguento mais',
    // General severity boosters
    'end it all', 'overdose', 'final goodbye', 'goodbye cruel world'
];

const HIGH_SEVERITY_KEYWORDS = [
    'suicide', 'kill myself', 'end my life', 'want to die', 'take my own life',
    'commit suicide', 'najiua', 'nataka kufa', 'je veux mourir', 'quiero morir',
    'quero morrer', 'overdose', 'final goodbye'
];

async function detectCrisis(text) {
    if (!text) return { isCrisis: false, severity: 'low' };
    const lower = text.toLowerCase().trim();
    let isCrisis = false;
    let highestSeverity = 'medium';

    for (const kw of CRISIS_KEYWORDS) {
        // Use substring match for multi-word phrases, and whole-word for single words
        if (kw.includes(' ')) {
            if (lower.includes(kw)) {
                isCrisis = true;
                if (HIGH_SEVERITY_KEYWORDS.some(hk => lower.includes(hk))) {
                    highestSeverity = 'high';
                }
                break;
            }
        } else {
            // For single words, match as whole word to avoid false positives (e.g., "I read about suicide")
    ...
    return { isCrisis, severity: isCrisis ? highestSeverity : 'low' };
}
