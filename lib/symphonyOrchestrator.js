...
Continue:`;
    ...
    buildAgentSystemPrompt(session, issue) {
        return `You are a Symphony coding agent working on GitHub issue #${issue.identifier}: "${issue.title}".
You are operating in workspace: ${session.workspace_path}
You have access to all Maureonix system commands (read_file, write_file, exec_cmd, etc.).
Your task is to analyze the issue, implement the fix or feature, test it, and report completion.
Rules:
1. Always backup files before modifying them
