...
class AgentRunner extends EventEmitter {
    ...
    async runAgentTurn(session, issue, attempt) {
        ...
        try {
            ...
            // We need a mock maureonix for non-WhatsApp contexts, or use the notification system
            const mockmaureonix = this.createMockmaureonix(session);
...
Continue:`;
    ...
    createMockmaureonix(session) {
        // Returns a mimaureonixl mock that captures messages for the notification system
        const notifications = [];
        return {
...
class NotificationSystem {
    constructor(config) {
        this.config = config;
        this.maureonix = global.maureonixInstance || null;
    }

    setmaureonix(instance) {
        this.maureonix = instance;
    }

    async sendWhatsApp(jid, text) {
        if (!this.config.notifications.whatsapp_enabled || !this.maureonix) return false;
        try {
            await this.maureonix.sendMessage(jid, { text });
            return true;
        } catch (e) {
