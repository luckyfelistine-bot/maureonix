...
const { mathEngine } = require('./mathBridge'); // optional, for math tasks
...
function addTask({ userId, description, runDate = null, dependencies = [], priority = 'normal', recurring = null }) {
    const task = {
        ...
        priority,
        created: Date.now(),
        runDate: runDate || null,    // if null, run immediately
        dependencies: dependencies || [],
        recurring: recurring || null, // { interval: 'daily|weekly|monthly', at: '09:00' }
        result: null,
        attempts: 0,
        maxAttempts: 3,
        progress: []
...
async function pollTasks() {
    ...
    const ready = tasks.filter(t => {
        if (t.status !== 'pending') return false;           // not waiting
        if (t.runDate && new Date(t.runDate).getTime() > now) return false; // not yet time
...
async function executeTask(task) {
    ...
    try {
        const AI = require('./ai');
        // Check if this is a Symphony-orchestrated task
        const isSymphonyTask = task.description.startsWith('symphony:');

        let systemPrompt;
        if (isSymphonyTask) {
            // Extract issue info from task description
            const issueMatch = task.description.match(/symphony:\s*#?\ *(\d+)/);
        ...
        while (currentRound < maxRounds) {
            ...
            if (actionMatches) {
                for (const match of actionMatches) {
                    ...
                    // Pass the action through the same executor used in maureonixCore
                    try {
                        const { executeActions } = require('./maureonixCore');
                        // executeActions expects a response text and a nimesha instance
                        // We don't have a real WhatsApp context, so we use a mock
                        // For now, skip complex actions that need a real socket
                        // In the future, extend with a mini‑executor
    ...
    try {
        const nimesha = global.nimaInstance;
        if (nimesha && task.userId) {
            ...
            if (task.status === 'completed') {
                await nimesha.sendMessage(ownerJid, { text: `✅ *Task completed:* ${task.description}\n\n📝 Result: ${task.result}` });
            } else if (task.status === 'failed' && task.attempts >= task.maxAttempts) {
                await nimesha.sendMessage(ownerJid, { text: `❌ *Task failed permanently:* ${task.description}\n\nError: ${task.result}` });
            }
        }
    } catch (e) {}
