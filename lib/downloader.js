...
/**
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║          MAUREONIX — ULTIMATE DOWNLOADER ENGINE  v6.0                  ║
 * ║                                                                          ║
 * ║  RESEARCH-VALIDATED  (April 2026)                                       ║
 * ║                                                                          ║
 * ║  YouTube  → youtubei.js  (YouTube InnerTube API — no Python, no bot)   ║
 * ║  TikTok   → TikWM API → yt-dlp fallback                                ║
 * ║  Spotify  → spotdl                                                     ║
 * ║  Social   → yt-dlp (2000+ sites) → direct HTTP                         ║
 * ║  Images   → gallery-dl                                                 ║
 * ║                                                                          ║
 * ║  REMOVED: Cobalt auto-discovery (instances.cobalt.best is dead)        ║
 * ║  ADDED  : youtubei.js for YouTube (bypasses yt-dlp bot blocks)         ║
 * ║  FIXED  : yt-dlp args for Python 3.10+ compatibility                   ║
 * ║                                                                          ║
 * ║  REQUIRED: nixpacks.toml with python310 (see header comment)           ║
 * ║  REQUIRED: npm install youtubei.js                                     ║
 * ╚═══════════════════════════════════════════════════════════════════════════╝
 */
...
const COBALT_API    = process.env.COBALT_API      || null   // YOUR self-hosted instance ONLY
const MAX_CONCURRENT= parseInt(process.env.DL_CONCURRENCY || '4', 10)
const TG_MAX_MB     = 49           // Telegram / WhatsApp file cap
const DL_TIMEOUT_MS = 360_000      // 6 min per single download
const MAX_RETRIES   = 3

// ─────────────────────────────────────────────────────────────────────────────
//  TOKEN MANAGER (auto‑refresh visitorData + poToken for YouTube)
// ─────────────────────────────────────────────────────────────────────────────
class YouTubeTokenManager {
  ...
  async getTokens() {
    if (this.visitorData && this.poToken && (Date.now() - this.lastRefresh) < this.refreshInterval) {
      return { visitorData: this.visitorData, poToken: this.poToken }
    }
    if (process.env.YT_VISITOR_DATA && process.env.YT_PO_TOKEN) {
      this.visitorData = process.env.YT_VISITOR_DATA
      this.poToken = process.env.YT_PO_TOKEN
    ...
    if (!this.refreshing) {
      ...
      try {
        const { visitorData, poToken } = await this._extractTokensFromYouTube()
        this.visitorData = visitorData
        this.poToken = poToken
        this.lastRefresh = Date.now()
        console.log('🔄 YouTube tokens refreshed successfully')
      } catch (err) {
        console.error('❌ Failed to refresh YouTube tokens:', err.message)
        if (!this.visitorData) throw new Error('No valid YouTube tokens available')
      } finally {
        this.refreshing = false
      }
    } else {
  ...
  async _extractTokensFromYouTube() {
    ...
    try {
      ...
      if (!visitorCookie) throw new Error('VISITOR_INFO1_LIVE cookie not found')
      const visitorData = visitorCookie.value
      const poToken = await page.evaluate(() => {
        return new Promise((resolve) => {
          const originalFetch = window.fetch
          window.fetch = (...args) => {
            if (args[0]?.includes('/youtubei/v1/player')) {
              return originalFetch(...args).then(async (res) => {
                const clone = res.clone()
                const json = await clone.json()
                const token = json?.context?.serviceIntegrityDimensions?.poToken
      ...
      if (!poToken) throw new Error('Could not extract poToken')
...
const ytTokenManager = new YouTubeTokenManager()

// ── YouTube InnerTube API with auto‑refreshing tokens ──────────────────────
let _youtubei = null
let _youtubeiSessionExpiry = 0

async function getYoutubei() {
  const { visitorData, poToken } = await ytTokenManager.getTokens()
  const now = Date.now()
  if (_youtubei && _youtubeiSessionExpiry > now) return _youtubei
  const { Innertube } = require('youtubei.js')
  _youtubei = await Innertube.create({
    retrieve_player: false,
    session: { visitor_data: visitorData, po_token: poToken },
  })
  _youtubeiSessionExpiry = now + 5 * 60 * 60 * 1000
  return _youtubei
}
...
function downloadHTTP(url, dest, redirectCount = 0) {
  return new Promise((resolve, reject) => {
    ...
    const proto = url.startsWith('https') ? https : http
    const req   = proto.get(url, { headers: { 'User-Agent': UA, 'Accept': '*/*', 'Accept-Language': 'en-US,en;q=0.9', Referer: url } }, res => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
...
// ─────────────────────────────────────────────────────────────────────────────
//  TIKWM  API  ← PRIMARY engine for TikTok
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  YOUTUBEI.JS  ← PRIMARY engine for YouTube (replaces yt-dlp for YT)
//  Uses YouTube's private InnerTube API — no Python, no bot detection
// ─────────────────────────────────────────────────────────────────────────────
async function youtubeiDownload(url, opts = {}) {
  ...
  // Determine extension from stream or fallback
  ...
  // Audio from youtubei.js may come as webm/m4a — convert to mp3 if needed
...
// ─────────────────────────────────────────────────────────────────────────────
//  COBALT  API  ← OPTIONAL: only if user sets COBALT_API env var
//  Auto-discovery REMOVED — instances.cobalt.best is dead/unreliable
// ─────────────────────────────────────────────────────────────────────────────
async function cobaltDownload(url, audioOnly = false, onProgress = null) {
  if (!COBALT_API) throw new Error('Cobalt not configured. Set COBALT_API env var to your self-hosted instance.')
...
// ─────────────────────────────────────────────────────────────────────────────
//  YT-DLP  (universal 2000+ sites — NOT used for YouTube or TikTok primary)
//  FIXED: Better args to avoid bot detection, added cookie simulation
// ─────────────────────────────────────────────────────────────────────────────
...
async function smartDownload(url, opts = {}) {
  ...
  // ── YouTube ── youtubei.js PRIMARY (bypasses yt-dlp bot blocks)
  if (platform === 'youtube') {
    try {
      ...
      try {
        ...
        throw new Error(`YouTube download failed. youtubei.js: ${e.message} | yt-dlp: ${e2.message}`)
      }
    }
  }

  // ── TikTok ── TikWM primary, Cobalt (manual only) secondary, yt-dlp last
  if (platform === 'tiktok') {
    try {
    ...
    if (COBALT_API) {
      try {
        log('🔷 Cobalt fallback (self-hosted)...')
        return await cobaltDownload(url, audioOnly, log)
      } catch (e2) {
        log(`⚠️ Cobalt failed: ${e2.message}`)
      }
    }
    try {
      log('⚡ yt-dlp fallback → TikTok...')
      return await ytdlpDownload(url, { audioOnly, quality })
    } catch (e3) {
      throw new Error(`TikTok download failed. TikWM: ${e3.message}`)
    }
  }
...
async function fetchURLs(query, opts = {}) {
  ...
  // ── YouTube via youtubei.js search (no API key, no yt-dlp) ───────────────
...
async function fetchRelated(exampleUrl, opts = {}) {
  ...
  // b) Platform-specific metadata extraction
  if (platform === 'youtube') {
    try {
