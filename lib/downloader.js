...
/**
 * ╔══════════════════════════════════════════════════════════════════════════════════════╗
 * ║     MAUREONIX  —  ULTIMATE DOWNLOADER ENGINE  v5.3  "TITAN"  (PATCHED)          ║
 * ║                                                                                      ║
 * ║  DIAGNOSTIC VERIFIED:                                                                ║
...
async function checkBinary(name, bin, args = ["--version"]) {
  ...
  try {
    ...
    const ok = { ok: true, version: stdout.trim().split(/\r?\n/)[0] };
...
function safeJoin(base, ...parts) {
  ...
  if (process.platform === "win32") {
    if (!resolved.toLowerCase().startsWith(baseResolved.toLowerCase())) throw new Error("Path traversal detected");
  } else {
    if (!resolved.startsWith(baseResolved)) throw new Error("Path traversal detected");
  }
  return resolved;
}
...
async function downloadHTTP(url, dest, opts = {}) {
  ...
  if (CFG.PROXY_URL) {
    try {
      ...
      axiosOpts.httpsAgent = new HttpsProxyAgent(CFG.PROXY_URL);
      axiosOpts.httpAgent = new HttpsProxyAgent(CFG.PROXY_URL);
    } catch {
  ...
  // Server ignored Range and sent full file — truncate to avoid corruption
  if (truncateOn200 && response.status === 200) {
    writer.destroy();
    const freshWriter = fs.createWriteStream(dest);
    response.data.pipe(freshWriter);
    ...
    await new Promise((resolve, reject) => {
      freshWriter.on("finish", resolve);
      freshWriter.on("error", reject);
...
function buildYtdlpArgs(url, opts = {}) {
  ...
  const tpl = outputTemplate || safeJoin(CFG.DL_DIR, "%(title).120s.%(ext)s");
  const args = [
    "--no-warnings",
...
async function ytdlpDownload(url, opts = {}) {
  if (cbYtdlp.isOpen) throw new Error("yt-dlp circuit breaker OPEN");
...
async function ytdlpBatch(urls, opts = {}) {
  if (cbYtdlp.isOpen) throw new Error("yt-dlp circuit breaker OPEN");
  await requireBinary("yt-dlp", CFG.YTDLP_BIN);
  return new Promise((resolve, reject) => {
...
//  SPOTDL
// ──────────────────────────────────────────────────────────────────────────────────────
async function spotdlDownload(url, onProgress = null) {
  if (cbSpotdl.isOpen) throw new Error("spotdl circuit breaker OPEN");
...
//  GALLERY-DL
// ──────────────────────────────────────────────────────────────────────────────────────
async function galleryDlDownload(url, onProgress = null) {
  if (cbGallery.isOpen) throw new Error("gallery-dl circuit breaker OPEN");
...
//  FFMPEG COMPRESS
// ──────────────────────────────────────────────────────────────────────────────────────
async function ffmpegCompress(src, targetMB = CFG.TG_MAX_MB) {
...
async function m3u8Download(url, opts = {}) {
  ...
  try {
    ...
    // Fallback to ffmpeg direct download
    try {
      await requireBinary("ffmpeg", CFG.FFMPEG_BIN);
    } catch (e) {
...
async function smartDownload(url, opts = {}) {
  ...
  // Loader.to for YouTube (best-effort; often blocked by anti-bot)
...
async function fetchURLs(query, opts = {}) {
  ...
  if (!site || /youtube/i.test(site) || type === "video" || type === "audio") {
    try {
      ...
      for (const u of stdout.trim().split(/\r?\n/).filter(Boolean)) results.add(u);
    } catch {}
  }
...
async function healthCheck() {
  ...
  const checkBin = async (name, bin, args = ["--version"]) => {
    try {
      const { stdout } = await execFileAsync(bin, args, { timeout: 10000 });
      checks[name] = { ok: true, version: stdout.trim().split(/\r?\n/)[0] };
    } catch (e) {
