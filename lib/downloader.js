...
// ═══════════════════════════════════════════════════════════════
async function ytdlpFallback(url, type = 'video') {
    ...
    const cmd = `yt-dlp ${format} --no-playlist --no-warnings --no-check-certificate -o "${outPath}" "${url}"`;
    
    try {
        await execAsync(cmd, { timeout: YTDL_TIMEOUT });
        if (fs.existsSync(outPath)) {
            const stats = fs.statSync(outPath);
            if (stats.size > 2048) return { url: outPath, local: true, title: 'Downloaded via yt-dlp', size: stats.size };
        }
    } catch (e) { 
        try { fs.unlinkSync(outPath); } catch {}
    }
    throw new Error('yt-dlp failed — make sure yt-dlp is installed: npm i -g yt-dlp');
}
...
async function ytMp3(url) {
    ...
    const apis = [
        ...
        `https://api.nyxs.pw/dl/yt-direct?url=${encodeURIComponent(url)}`,
    ];
    
    try {
        return await fetchWithFallback(apis, (data) => {
            if (data?.status && data.data?.dl) return { url: data.data.dl, title: data.data.title || 'YouTube Audio' };
            ...
            if (data?.status === 200 && data.result?.dl) return { url: data.result.dl, title: data.result.title };
            return null;
        });
    } catch {
...
async function ytMp4(url) {
    ...
    try {
        return await fetchWithFallback(apis, (data) => {
            if (data?.status && data.data?.dl) return { url: data.data.dl, title: data.data.title || 'YouTube Video' };
    ...
    try {
        return await fetchWithFallback(apis, (data) => {
            ...
            if (data?.status === 200 && data.data) {
                if (data.data?.video) return { type: 'video', url: data.data.video, title: data.data.title || 'TikTok' };
                if (data.data?.images) return { type: 'images', items: data.data.images, title: data.data.title || 'TikTok' };
            }
            if (data?.code === 0) {
...
async function igDownload(url) {
    ...
    try {
        return await fetchWithFallback(apis, (data) => {
            ...
            if (data?.status === 200 && data.data?.url) return { type: 'video', url: data.data.url };
...
async function twitterDownload(url) {
    ...
    try {
        return await fetchWithFallback(apis, (data) => {
            ...
            if (data?.status === 200 && data.data?.url) return { url: data.data.url, title: data.data.title || 'Twitter Video' };
...
async function spotifyDownload(url) {
    ...
    const apis = [
        ...
        `https://api.agatz.xyz/api/spotify?url=${encodeURIComponent(url)}`,
    ];
...
async function vimeoDownload(url) {
    ...
    try {
        const { data } = await axios.get(`https://api.siputzx.my.id/api/d/vimeo?url=${encodeURIComponent(url)}`, { timeout: API_TIMEOUT });
        if (data?.status && data.data?.url) return { url: data.data.url, title: data.data.title || 'Vimeo Video' };
    } catch {}
...
async function dailymotionDownload(url) {
    ...
    try {
        const { data } = await axios.get(`https://api.siputzx.my.id/api/d/dailymotion?url=${encodeURIComponent(url)}`, { timeout: API_TIMEOUT });
        if (data?.status && data.data?.url) return { url: data.data.url, title: data.data.title || 'Dailymotion Video' };
    } catch {}
