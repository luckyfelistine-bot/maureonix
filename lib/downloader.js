...
/**
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║          MAUREONIX — DEBUGGED DOWNLOADER ENGINE  v7.4                  ║
 * ║                                                                          ║
 * ║  CRITICAL FIXES:                                                         ║
 * ║  1. DL_CONCURRENCY NaN guard → defaults to 3                             ║
 * ║  2. downloadHTTP maxRedirects=10, content-type guard, size limit         ║
 * ║  3. tiktokScrapeDownload checks urlList AND UrlList                      ║
 * ║  4. youtubeiDownload cleanup on ffmpeg failure                           ║
 * ║  5. ytdlpDownload --no-colors flag                                       ║
 * ║  6. tikwmDownload validates data.music is string                         ║
 * ╚═══════════════════════════════════════════════════════════════════════════╝
 */
...
// CRITICAL FIX #1: NaN guard for DL_CONCURRENCY
...
const MAX_RETRIES    = 3
const MAX_DOWNLOAD_MB = 500  // 500MB hard limit per file
...
// ─────────────────────────────────────────────────────────────────────────────
//  DIRECT HTTP DOWNLOADER  (CRITICAL FIX #2)
//  - maxRedirects: 10 (was default 5)
...
async function downloadHTTP(url, dest, opts = {}) {
  ...
  const { expectedType = 'binary', maxSizeMB = MAX_DOWNLOAD_MB } = opts
...
// ─────────────────────────────────────────────────────────────────────────────
//  TIKTOK DIRECT SCRAPER  (CRITICAL FIX #3: urlList + UrlList)
// ─────────────────────────────────────────────────────────────────────────────
async function tiktokScrapeDownload(url, audioOnly = false, onProgress = null) {
  ...
  // Try __UNIVERSAL_DATA_FOR_REHYDRATION__ first, then SIGI_STATE
  ...
  if (!scriptMatch) {
    throw new Error('TikTok scrape: no data script found (blocked or page changed)')
  }
  ...
  try {
    ...
    throw new Error('TikTok scrape: failed to parse page data JSON')
  }
  ...
  if (!itemStruct) {
    throw new Error('TikTok scrape: video data not found in page')
  }
  ...
  // CRITICAL FIX #3: Check urlList, UrlList, urls, and direct string
  const getUrl = (val) => {
...
// ─────────────────────────────────────────────────────────────────────────────
//  TIKWM API  (CRITICAL FIX #6: validate data.music is string)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  YOUTUBEI.JS  (CRITICAL FIX #4: cleanup on ffmpeg failure)
// ─────────────────────────────────────────────────────────────────────────────
async function youtubeiDownload(url, opts = {}) {
  ...
  if (audioOnly && !dest.endsWith('.mp3')) {
    ...
    try {
      ...
      // CRITICAL FIX #4: Always cleanup source file, even if ffmpeg fails
      cleanupFile(dest)
    }
    return [mp3Dest]
  }
...
// ─────────────────────────────────────────────────────────────────────────────
//  YT-DLP  (CRITICAL FIX #5: --no-colors)
// ─────────────────────────────────────────────────────────────────────────────
function buildYtdlpArgs(url, opts = {}) {
  ...
  const args = [
    '--no-warnings',
    '--no-colors',                    // CRITICAL FIX #5: prevent ANSI in output
    ...
    const schedule = () => {
      while (active.size < MAX_CONCURRENT && queue.length > 0) {
        ...
        const task = smartDownload(url, {
          audioOnly, quality,
          onProgress: m => { try { onProgress?.({ done, total, url, status: 'progress', message: m }) } catch {} },
        })
