...
function discoverSkillsFromDir(dirPath, baseRoot = dirPath, depth = 0) {
    ...
    // Server / entry files that must never be required a second time
    const skipFiles = ['index.js', 'maureonix.js', 'start.js', 'server.js', 'maureonix_core.js', 'maureonix_commands.js'];
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            // Skip entry files that start servers or cause duplicate port binding
            ...
            try {
                ...
                // Module couldn't be loaded (missing deps, syntax error, etc.) – ignore
            }
        }
    }
