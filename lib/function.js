...
const axiosss = axios.create({ httpsAgent: new https.Agent({ rejectUnauthorized: false, keepAlive: false }) });
...
async function updateSettings({ filePath, owner, author, apikey, botname, packname }) {
    return new Promise((resolve, reject) => {
        try {
            ...
            if (apikey) content = content.replace(/(global\.APIKeys\s*=\s*\{[\s\S]*?'\https:\/\/api\.maureonix\.biz\.id'\s*:\s*')[^']*(')/, `$1${apikey}$2`);
...
function assertInstalled(cmd, name, code) {
    try {
        ...
        console.error(chalk.redBright(`❌  ${name} is not installed.`) + `\nPlease install it first and try again.\n`);
        process.exit(code);
    }
}
