...
class ProactiveIntelligence {
    ...
    async generateInsights() {
        ...
        if (ctx.pendingReminders > 5) {
            insights.push({ type: 'reminder_backlog', message: `You have ${ctx.pendingReminders} pending reminders. Want me to prioritise them?`, priority: 'medium' });
        }
        if (ctx.hour > 23 || ctx.hour < 5) {
           insights.push({ type: 'bedtime', message: "It's late. Consider getting some rest.", priority: 'low' });
        }
        //if (ctx.memoryPressure > 0.85) {
        //    insights.push({ type: 'memory_warning', message: 'I\'ve automatically trimmed my memory to keep the bot fast.', priority: 'medium' });
        //}
        return insights;
    }
    async executeProactive() {
        ...
        for (const ins of insights.slice(0, 2)) {
            if (state.nimesha && state.ownerJid) {
                try {
                    await state.nimesha.sendMessage(state.ownerJid, { text: `💫 *${ins.type.replace(/_/g, ' ')}*\n\n${ins.message}` });
...
async function checkAnomalies() {
    ...
    try {
        ...
        // Do NOT report warnings – keep them silent
    } catch (e) {}
}
