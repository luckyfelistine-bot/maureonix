...
class ReminderSystem {
    ...
    async checkAndNotify() {
        ...
        for (const [id, r] of this.reminders) {
            ...
            r.notified = true;
            if (state.maureonix && state.ownerJid) {
                try {
                    await state.maureonix.sendMessage(state.ownerJid, { text: `⏰ *Reminder*: ${r.text}` });
                } catch {}
            }
        }
    }
...
class ProactiveIntelligence {
    ...
    async executeProactive() {
        ...
        for (const ins of insights.slice(0, 2)) {
            if (state.maureonix && state.ownerJid) {
                try {
                    await state.maureonix.sendMessage(state.ownerJid, {
...
async function sendDailyBriefing() {
    if (!state.maureonix || !state.ownerJid) return;
    ...
    try { await state.maureonix.sendMessage(state.ownerJid, { text: msg }); state.healthMetrics.lastBriefing = today; } catch {}
}

async function checkAnomalies() {
    if (!state.maureonix) return;
    try {
        ...
        if (critical.length > 0) {
            ...
            if (!state.shortTermMemory.has(cacheKey)) {
                ...
                await state.maureonix.sendMessage(state.ownerJid, { text: msg });
            }
        }
    } catch (e) {}
}
...
async function checkWeatherAlert() {
    ...
    try {
        ...
        if ([95, 96, 99].includes(code)) {
            await state.maureonix.sendMessage(state.ownerJid, { text: `⛈️ *Severe weather alert!* Thunderstorms expected today.` });
        }
...
async function sendLearningDigest() {
    ...
    try {
        ...
        if (total > 0) {
            await state.maureonix.sendMessage(state.ownerJid, { text: `📚 *Learning Digest*\n\nModules: ${total}\nMastered: ${mastered} (${Math.round(mastered/total*100)}%)\n\nKeep learning! 🧠` });
...
async function checkBackupAge() {
    ...
    try {
        ...
        if (!backups.length) {
            await state.maureonix.sendMessage(state.ownerJid, { text: '⚠️ *No backups found.* Run .backupnow.' });
        } else {
            const latest = backups[0].created;
            const hrs = (Date.now() - latest.getTime()) / 3600000;
            if (hrs > 24) {
                await state.maureonix.sendMessage(state.ownerJid, { text: `⚠️ *Last backup ${Math.round(hrs)}h ago.*` });
            }
        }
...
async function sendWeeklyTrendReport() {
    ...
    try {
        ...
        await state.maureonix.sendMessage(state.ownerJid, { text: `📈 *Weekly Health Trend*\n\n${res.text}` });
        state.healthMetrics.lastTrendReport = weekKey;
    } catch {}
}

async function crisisFollowUp() {
    if (!state.maureonix || !global.db?.crisisPending) return;
    const now = Date.now();
    for (const [userId, cs] of Object.entries(global.db.crisisPending)) {
        if (cs.state === 'talking' && now - cs.lastMsgTime > 30 * 60 * 1000) {
            try { await state.maureonix.sendMessage(userId, { text: '💙 Just checking in. I\'m still here.' }); cs.lastMsgTime = now; } catch {}
        }
    }
}
...
async function selfDiagnose() {
    ...
    try {
        ...
        if (diag.status === 'CRITICAL') {
            await state.maureonix.sendMessage(state.ownerJid, { text: `🛑 *Self‑diagnosis CRITICAL!* Check logs.` });
        }
...
function scheduleProactiveMessage(delayMinutes, contact, text) {
    if (!state.maureonix) return false;
    setTimeout(() => {
        if (state.maureonix) state.maureonix.sendMessage(contact, { text }).catch(() => {});
    }, delayMinutes * 60000);
    return true;
}
...
async function symphonyHealthCheck() {
    try {
        const { symphonyOrchestrator } = require('./symphonyOrchestrator');
        const status = symphonyOrchestrator.getStatus();

        if (status.retrying_count > 5) {
            await state.maureonix.sendMessage(state.ownerJid, {
                text: `⚠️ *Symphony Alert*\n\n${status.retrying_count} issues are stuck in retry queue. Check GitHub or WORKFLOW.md configuration.`
            });
        }
        if (!status.is_running && status.running_count > 0) {
            await state.maureonix.sendMessage(state.ownerJid, {
                text: `🚨 *Symphony Critical*\n\nOrchestrator stopped but ${status.running_count} sessions still marked running. Manual intervention needed.`
            });
        }
    } catch (e) {}
}
...
//  Initialisation
// ═══════════════════════════════════════════════════════════════
async function init(maureonixInstance) {
    if (state.isInitialized) return;
    state.maureonix = maureonixInstance;
    ...
    cron.schedule('0 9 * * 1', () => safeExecute('weeklyTrend', sendWeeklyTrendReport), { timezone: ENGINE_CONFIG.TIMEZONE });
        // ── Symphony health check every 10 minutes ──
    cron.schedule('*/10 * * * *', () => safeExecute('symphonyHealth', symphonyHealthCheck), { timezone: ENGINE_CONFIG.TIMEZONE });


    // ── Daily code audit at 3 AM ──
    cron.schedule('0 3 * * *', () => safeExecute('dailyAudit', async () => {
        try {
            ...
            if (audit.analysis.criticalIssues > 0 && state.maureonix && state.ownerJid) {
                await state.maureonix.sendMessage(state.ownerJid, {
                    text: `🚨 *Daily Code Audit*\n\nScore: ${audit.analysis.averageScore}/100\nCritical: ${audit.analysis.criticalIssues}\n\n${audit.analysis.recommendation}`
                });
            }
        } catch {}
    }), { timezone: ENGINE_CONFIG.TIMEZONE });

    // ── Weekly creative synthesis on Sundays at 10 AM ──
    cron.schedule('0 10 * * 0', () => safeExecute('weeklyCreative', async () => {
        try {
            const { superIntelligence } = require('./superIntelligencePack');
            if (!superIntelligence.initialized) await superIntelligence.initialize();
            const ideas = await superIntelligence.creative.brainstorm('improve the bot architecture');
            if (ideas.length > 0 && state.maureonix && state.ownerJid) {
                await state.maureonix.sendMessage(state.ownerJid, {
                    text: `💡 *Weekly Creative Synthesis*\n\nTop idea:\n${ideas[0].idea}\n\nNovelty: ${Math.round(ideas[0].novelty * 100)}%`
                });
            }
        } catch {}
    }), { timezone: ENGINE_CONFIG.TIMEZONE });
    ...
    const intervals = [
        ...
        { name: 'symphonyHealth', ms: 10 * 60000, fn: symphonyHealthCheck },
        { name: 'symphonyHealth', ms: 10 * 60000, fn: symphonyHealthCheck },
        { name: 'neuralMemoryConsolidation', ms: 30 * 60000, fn: async () => {
            try {
                const { superIntelligence } = require('./superIntelligencePack');
                if (superIntelligence.initialized) {
                    const consolidated = await superIntelligence.memory.consolidate();
                    if (consolidated > 0) {
                        console.log(`[SuperIntelligence] Consolidated ${consolidated} memories`);
                    }
                }
            } catch {}
        }},
        { name: 'predictiveMaintenance', ms: 10 * 60000, fn: async () => {
            try {
                const { superIntelligence } = require('./superIntelligencePack');
                if (superIntelligence.initialized) {
                    const alerts = await superIntelligence.predictAndPrevent();
                    if (alerts.length > 0 && state.maureonix && state.ownerJid) {
                        for (const alert of alerts) {
                            if (alert.severity === 'high') {
                                await state.maureonix.sendMessage(state.ownerJid, {
                                    text: `🔮 *Predictive Alert*\n\n${alert.message}\n\nAction: ${alert.action}`
                                });
                            }
                        }
                    }
        ...
        { name: 'selfReflection', ms: 60 * 60000, fn: async () => {
            try {
                const { superIntelligence } = require('./superIntelligencePack');
                if (superIntelligence.initialized) {
                    await superIntelligence.selfImprove();
                    console.log('[SuperIntelligence] Self-reflection complete');
                }
            } catch {}
        }},
    ];
...
process.on('SIGTERM', async () => {
    ...
    state.timers.clear();
    if (state.maureonix && state.ownerJid) {
        await state.maureonix.sendMessage(state.ownerJid, { text: '🔌 *Consciousness saved.* Until next time, Creator.' }).catch(() => {});
    }
    process.exit(0);
});
