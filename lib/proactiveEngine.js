...
async function init(nimeshaInstance) {
    ...
    cron.schedule('0 3 * * *', () => safeExecute('dailyAudit', async () => {
        try {
            ...
            if (audit.analysis.criticalIssues > 0 && state.nimesha && state.ownerJid) {
                await state.nimesha.sendMessage(state.ownerJid, {
    ...
    cron.schedule('0 10 * * 0', () => safeExecute('weeklyCreative', async () => {
        try {
            ...
            if (ideas.length > 0 && state.nimesha && state.ownerJid) {
                await state.nimesha.sendMessage(state.ownerJid, {
        ...
        { name: 'predictiveMaintenance', ms: 10 * 60000, fn: async () => {
            try {
                ...
                if (superIntelligence.initialized) {
                    ...
                    if (alerts.length > 0 && state.nimesha && state.ownerJid) {
