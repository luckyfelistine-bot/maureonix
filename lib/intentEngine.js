// ═══════════════════════════════════════════════════════════════════════════
//   🦊 MAUREONIX ORACLE INTENT ENGINE v4.1 — "THE MIND READER"
//   Semantic Embedding · State Machine · Intent Fusion · Temporal Parsing
//   Cross-Lingual Zero-Shot · Confidence Calibration · Fast Rule‑Based Matching
// ═══════════════════════════════════════════════════════════════════════════

const fetch = require('node-fetch');

// ═══════════════════════════════════════════════════════
//  GOOGLE TRANSLATE (lightweight copy)
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
//  SEMANTIC INTENT MATCHER (with caching)
// ═══════════════════════════════════════════════════════
class SemanticIntentMatcher {
    ...
    async generateEmbedding(text) {
        ...
        const embedding = {};
        for (const word of words) embedding[word] = (embedding[word] || 0) + 1;
    ...
    cosineSimilarity(emb1, emb2) {
        ...
        for (const key of allKeys) {
            const v1 = emb1[key] || 0, v2 = emb2[key] || 0;
...
// ═══════════════════════════════════════════════════════
//  FAST RULE-BASED MATCHER (no AI call)
// ═══════════════════════════════════════════════════════
class FastRuleMatcher {
    constructor(taxonomy) {
        this.taxonomy = taxonomy;
        // precompile regex from verbs and nouns
        this.rules = taxonomy.map(item => ({
            cmd: item.cmd,
            regex: new RegExp(`\\b(${item.verbs.concat(item.nouns).map(v => v.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`, 'i')
        }));
    }
    match(text) {
        ...
        if (prefixMatch) {
            ...
            if (this.taxonomy.some(t => t.cmd === cmd)) {
                return { cmd, confidence: 'certain', source: 'prefix' };
            }
        }
        // explicit command mention
        const explicitMatch = lower.match(/(?:run|do|execute|use|call|trigger)\s+(?:\.|!|\?)?(\w+)/);
        if (explicitMatch && this.taxonomy.some(t => t.cmd === explicitMatch[1])) {
            return { cmd: explicitMatch[1], confidence: 'high', source: 'explicit' };
        }
        // find best rule match
        let bestScore = 0, bestCmd = null;
        for (const rule of this.rules) {
            const match = rule.regex.test(lower);
            if (match) {
                // score based on number of matches (just one match detected, still boost)
                bestCmd = rule.cmd;
                bestScore = 0.7; // medium confidence from rule
                break; // take first match for simplicity, could aggregate later
            }
        }
        if (bestCmd) return { cmd: bestCmd, confidence: 'medium', source: 'rule' };
        return null;
    }
}
...
class ConversationStateMachine {
    ...
    getStateContext(userId) {
        const state = this.getState(userId);
        return { current: state.currentState, duration: Date.now() - state.enteredAt, history: state.history.slice(-5), data: state.stateData };
    }
    reset(userId) { this.states.delete(userId); }
}
...
class IntentFusionEngine {
    fuse(signals) {
        ...
        for (const [intent, sigs] of Object.entries(grouped)) {
            ...
            fused.push({ intent, confidence: Math.min(1.0, weightedConf * agreement), sources: sigs.map(s => s.source), rawSignals: sigs.length });
        }
...
class TemporalIntentParser {
    parse(text) {
        const patterns = [
            ...
            { regex: /\b(in\s+(\d+)\s*(min|minute|minutes|hour|hours|hr|hrs|day|days))\b/i, type: 'delayed', delay: (m) => { const num = parseInt(m[2]); const unit = m[3].startsWith('min') ? 60 : m[3].startsWith('hour') || m[3].startsWith('hr') ? 3600 : 86400; return num * unit * 1000; } },
        ...
        for (const p of patterns) {
            ...
            if (match) {
                return { hasTemporal: true, type: p.type, delay: p.delay ? p.delay(match) : null, raw: p.extract ? p.extract(match) : match[0] };
            }
        }
...
// ═══════════════════════════════════════════════════════
//  ORACLE INTENT ENGINE (optimized)
// ═══════════════════════════════════════════════════════
class IntentEngine {
    constructor(opts = {}) {
    ...
    async contextualMatch(text) {
        // 1) Try fast rule-based match (no AI)
        const fast = this.fastMatcher.match(text);
        if (fast && fast.confidence === 'certain') {
            return { cmd: fast.cmd, args: [], confidence: 'certain', source: fast.source };
        }
        // 2) Semantic matching (still lightweight)
        const semanticMatches = await this.semanticMatcher.match(text, 1);
        if (semanticMatches.length > 0 && semanticMatches[0].score > 0.85) {
            return { cmd: semanticMatches[0].intent.cmd, args: [], confidence: 'certain', source: 'semantic' };
        }
        if (fast) {
            return { cmd: fast.cmd, args: [], confidence: 'high', source: fast.source };
        }
        ...
        if (/remind me (to|at|in|about|on)/i.test(lower) || /set a reminder/i.test(lower)) {
            ...
            return { cmd: 'remind', args: [reminderText], confidence: 'certain', source: 'reminder_pattern' };
        }
        return null;
    }
...
OUTPUT JSON ONLY:
{
  ...
  "response": {"text":"reply if conversation or clarification"}
        ...
        try {
            const { keyManager, groqChat } = require('./ai');
            const apiKey = keyManager.getNext();
            const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({ model: this.model, messages: [{ role: 'user', content: prompt }], temperature: 0.05, max_tokens: 600 }),
            });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            const data = await res.json();
            ...
            if (intentType === 'function' && conf === 'certain' && parsed.intent.function) {
                ...
                return { type: 'function', function: parsed.intent.function, args: Array.isArray(parsed.intent.args) ? parsed.intent.args : [], confidence: 'certain', source: 'llm_certain', temporal };
            }
            if ((conf === 'likely' || conf === 'uncertain') && intentType === 'function') {
                ...
                return { type: 'text', text: parsed.response?.text || `Did you mean ".${parsed.intent.function}"?`, confidence: conf, source: 'llm_conservative', suggestedCommand: parsed.intent.function, temporal };
            }
            if (intentType === 'game') {
                return { type: 'game', game: parsed.intent.function, confidence: conf, source: 'llm_game' };
            }
            this.stateMachine.transition(this.userId, 'conversing');
            return { type: 'text', text: parsed.response?.text || null, confidence: conf, source: 'llm_conversation' };
        } catch (e) {
    ...
    async preprocess(text) {
        ...
        for (const [lang, pattern] of Object.entries(langPatterns)) {
            if (pattern.test(lower)) { detectedLang = lang; break; }
        }
        if (detectedLang !== 'en') {
    ...
    async parse(text) {
        ...
        // 4. Contextual match (fast, then semantic)
        const contextual = await this.contextualMatch(normalized);
        if (contextual) {
            const result = { ...contextual, detectedLang };
            if (contextual.confidence === 'certain') this.stateMachine.transition(this.userId, 'executing', { command: contextual.cmd });
            return result;
        }

        // 5. LLM disambiguation (only if not sure)
