...
const gameSlot = async (conn, m, db) => {
	...
	const slots = ['🍇','🍉','🍋','🍌','🍎','🍑','🍒','🫐','🥥','🥑']
	const slot1 = pickRandom(slots)
	const slot2 = pickRandom(slots)
	const slot3 = pickRandom(slots)
	const listSlot1 = `${pickRandom(slots)} : ${pickRandom(slots)} : ${pickRandom(slots)}`
	const listSlot2 = `${slot1} : ${slot2} : ${slot3}`
	const listSlot3 = `${pickRandom(slots)} : ${pickRandom(slots)} : ${pickRandom(slots)}`
	...
	try {
		if (slot1 === slot2 && slot2 === slot3) {
			...
			let sloth =`[  🎰 VIRTUAL SLOT 🎰  ]\n------------------------\n\n${listSlot1}\n${listSlot2} <=====\n${listSlot3}\n\n------------------------\n[  🎰 VIRTUAL SLOT 🎰  ]\n\n*Result* :\n_You won!🎉_ <=====Limit + ${randomLimit}, Money + ${randomLimit * 500}`
...
const gameCasinoSolo = async (conn, m, prefix, db) => {
	try {
		...
		let randomUser = `${Math.floor(Math.random() * 81)}`.trim() 
		let botScore = (randomBot * 1)
		let userScore = (randomUser * 1)
		let count = m.args[0]
		count = count ? 'all' === count ? Math.floor(db.users[m.sender].money / bet) : parseInt(count) : m.args[0] ? parseInt(m.args[0]) : 1
		...
		if (db.users[m.sender].money >= count * 1) {
			...
			if (botScore > userScore) {
				m.reply(`💰 CASINO 💰\n*You:* ${userScore} points\n*Computer:* ${botScore} points\n\n*You lost*\nYou lost ${count} money.`.trim())
			} else if (botScore < userScore) {
				db.users[m.sender].money += count * 2
				m.reply(`💰 CASINO 💰\n*You:* ${userScore} points\n*Computer:* ${botScore} points\n\n*You won*\nYou received ${count * 2} money.`.trim())
			} else {
				db.users[m.sender].money += count * 1
				m.reply(`💰 CASINO 💰\n*You:* ${userScore} points\n*Computer:* ${botScore} points\n\n*Draw*\nYour money (${count}) returned.`.trim())
			}
...
const gameSamgongSolo = async (conn, m, db) => {
	...
	let result = playerScore > 30 ? '💀 You lost!' : playerScore === botScore ? '🤝 Draw! Bet returned.' : botScore > 30 || playerScore > botScore ? `🎉 You won! +${winnings} 💵` : '😞 Bot won!';
...
const gameMerampok = async (m, db) => {
	...
	} else m.reply(`You are already hiding after a robbery, wait another ${timers} to rob again.`)
}
...
const gameBegal = async (conn, m, db) => {
	...
	let random = [{teks: 'Player escaped successfully!', no: 0},{teks: 'Player ran away!', no: 0},{teks: 'Player is hiding', no: 0},{teks: 'Player committed suicide', no: 2},{teks: 'Player caught successfully', no: 2},{teks: 'Player could not be found!', no: 0},{teks: 'Player is stronger than you!', no: 1},{teks: 'Player cheated', no: 1},{teks: 'Player reported to police', no: 0},{teks: 'Player arrested!', no: 2},{teks: 'Player surrendered', no: 2}]
	...
	if (new Date - user.lastbegal > 3600000) {
		...
		if (teksnya.no === 0) {
			...
			await m.reply('Failed to find player, please try again.')
	...
	} else m.reply(`Please wait *⏱️${timers}* to play again.`)
}
...
const daily = async (m, db) => {
	...
	} else m.reply(`Please wait *⏱️${timers}* to claim again.`)
}
...
const buy = async (m, args, db) => {
	if (args[0] === 'limit') {
		...
		} else m.reply(`You don't have enough money to buy!\nRemaining money: ${db.users[m.sender].money}\nPrice: ${args[1] * 500}`);
...
class Blackjack {
	...
	reuseSubmitCardsForDrinking() {
		...
		if ((this.submitCard.length + this.skip.length) === this.players.length && cards.length === 1) {
			...
			return {
				msg: `@${owner.split('@')[0]} is the only one with cards, becomes the new leader.`,
				continue: true
			}
		} else {
			let index = 0;
