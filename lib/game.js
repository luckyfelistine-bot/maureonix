...
const gameCasinoSolo = async (conn, m, prefix, db) => {
    try {
        ...
        if (isNaN(m.args[0])) return m.reply(`Please enter an amount!\nExample: ${prefix + m.command} 1000`);
        if (db.users[m.sender].money >= count * 1) {
            ...
            if (Aku > Kamu) {
                ...
                db.users[m.sender].money += count * 1;
                m.reply(`💰 CASINO 💰\n*You:* ${Kamu} points\n*Computer:* ${Aku} points\n\n*Draw*\nYour money (${count}) is returned.`.trim());
            }
        } else m.reply(`You don't have enough money to play casino!`);
    } catch (e) {
        m.reply('Error!');
    }
};
...
const gameSamgongSolo = async (conn, m, db) => {
    ...
    if (db.users[m.sender].money < count) return m.reply(`You don't have enough money to play Samgong!`);
    ...
    let result = playerScore > 30 ? '💀 You lose!' : playerScore === botScore ? '🤝 Draw! Bet returned.' : botScore > 30 || playerScore > botScore ? `🎉 You win! +${winnings} 💵` : '😞 Bot wins!';
...
const gameMerampok = async (m, db) => {
    ...
    if (new Date - db.users[m.sender].lastrampok > 3600000) {
        ...
        if (!who) return m.reply('Please tag a person.');
        if (!db.users[who]) return m.reply('This person is not in the database!');
        if (10000 > db.users[who].money) return m.reply('They do not have enough money to rob.🗿');
    ...
    } else m.reply(`You are already hiding after a robbery. Please wait ${timers} before robbing again.`);
};
...
const gameBegal = async (conn, m, db) => {
    ...
    let random = [{teks: 'The player successfully escaped!', no: 0},{teks: 'The player ran away!', no: 0},{teks: 'The player is hiding', no: 0},{teks: 'The player committed suicide', no: 2},{teks: 'The player was caught successfully', no: 2},{teks: 'The player could not be found!', no: 0},{teks: 'The player is stronger than you!', no: 1},{teks: 'The player cheated!', no: 1},{teks: 'The player called the police', no: 0},{teks: 'The player was arrested!', no: 2},{teks: 'The player surrendered', no: 2}];
    ...
    if (new Date - user.lastbegal > 3600000) {
        ...
        if (teksnya.no === 0) {
            ...
            await m.reply({ text: teksnya.teks, edit: key });
            await m.reply(`You were killed by the player!\nYour money *${randomUang}* was robbed.`);
            db.users[m.sender].money -= randomUang;
            db.set[botNumber].money += randomUang * 1;
        } else {
            await m.reply({ text: teksnya.teks, edit: key });
            await m.reply(`Successfully obtained money: *${randomUang}*`);
            db.users[m.sender].money += randomUang;
...
const daily = async (m, db) => {
    ...
    if (new Date - user.lastclaim > 86400000) {
        m.reply(`*Daily rewards*\n_Successfully claimed_\n- Limit : 10\n- Money : 10000\n\n_Rewards reset._`);
...
const buy = async (m, args, db) => {
    if (args[0] === 'limit') {
        ...
        if (db.users[m.sender].money >= count * 500) {
            ...
            m.reply(`Successfully purchased ${args[1]} limit. Price: ${args[1] * 500}`);
        } else m.reply(`You don't have enough money to purchase!\nRemaining money: ${db.users[m.sender].money}\nPrice: ${args[1] * 500}`);
...
const transfer = async (m, args, db) => {
    if (args[0] == 'limit') {
        ...
        if (db.users[who]) {
            ...
            } else m.reply(`Not enough limit!!\nYour limit: *${db.users[m.sender].limit}*`);
        } else m.reply(`${who.split('@')[0]} is not in the database!`);
        ...
        if (db.users[who]) {
            ...
            } else m.reply(`Not enough money!!\nYour money: *${db.users[m.sender].money}*`);
        } else m.reply(`${who.split('@')[0]} is not in the database!`);
...
class Blackjack {
    ...
    reuseSubmitCardsForDrinking() {
        ...
        if ((this.submitCard.length + this.skip.length) === this.players.length && cards.length === 1) {
            ...
            return { msg: `@${owner.split('@')[0]} is the only one with cards, becomes the new leader.`, continue: true };
        } else {
            let index = 0;
...
class SnakeLadder {
    ...
    createMap () {
        const data = [{
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map1.jpg',
            move: { 4: 56, 12: 50, 14: 55, 22: 58, 41: 79, 54: 88, 96: 42, 94: 71, 75: 32, 48: 16, 37: 3, 28: 10 },
            mode: ''
        }, {
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map2.jpg',
            move: { 7: 36, 21: 58, 31: 51, 34: 84, 54: 89, 63: 82, 96: 72, 78: 59, 66: 12, 56: 20, 43: 24, 33: 5 },
            mode: ''
        }, {
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map3.jpg',
            move: { 8: 29, 10: 32, 20: 39, 27: 85, 51: 67, 72: 91, 79: 100, 98: 65, 94: 75, 93: 73, 64: 60, 62: 19, 56: 24, 53: 50, 17: 7 },
            mode: ''
        }, {
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map4.jpg',
            move: { 8: 29, 10: 32, 20: 39, 27: 85, 51: 67, 72: 91, 79: 100, 98: 65, 94: 75, 93: 73, 64: 60, 62: 19, 56: 24, 53: 50, 17: 7 },
            mode: ''
        }, {
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map5.jpg',
            move: { 1: 38, 4: 14, 9: 31, 21: 42, 28: 84, 51: 67, 72: 91, 80: 99, 98: 79, 94: 75, 93: 73, 87: 36, 64: 60, 62: 19, 54: 34, 17: 7 },
            mode: ''
        }, {
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map6.jpg',
            move: { 4: 23, 13: 46, 33: 52, 42: 63, 50: 69, 62: 81, 74: 93, 99: 41, 95: 76, 89: 53, 66: 45, 54: 31, 43: 17, 40: 2, 27: 5 },
            mode: ''
        }, {
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map7.jpg',
            move: { 1: 38, 4: 14, 9: 31, 21: 42, 28: 84, 51: 67, 71: 91, 80: 100, 98: 79, 95: 75, 93: 73, 87: 24, 64: 60, 62: 19, 54: 34, 17: 7 },
            mode: ''
        }, {
            url: 'https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/map/map8.jpg',
    ...
    async drawBoard(boardUrl, players = []) {
        try {
            ...
            for (let i = 0; i < players.length; i++) {
                ...
                const player = await jimp.read(`https://raw.githubusercontent.com/luckyfelistine-bot/maureonix/main/database/player/player${i + 1}.png`);
                const pionSize = tileSize * 0.7;
                player.resize(pionSize, pionSize);
                board.composite(player, x + tileSize / 2 - pionSize / 2, y + tileSize / 2 - pionSize / 2, { mode: jimp.BLEND_SOURCE_OVER });
            }
