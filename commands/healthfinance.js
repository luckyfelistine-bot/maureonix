...
module.exports = {
    // Health
    bmi: async (maureonix, m, { args, Health, prefix, command }) => {
    ...
    bmr: async (maureonix, m, { args, Health, prefix, command }) => {
    ...
    tdee: async (maureonix, m, { args, Health, prefix, command }) => {
    ...
    macros: async (maureonix, m, { args, Health, prefix, command }) => {
    ...
    onerm: async (maureonix, m, { args, Health, prefix, command }) => {
    ...
    bodyfat: async (maureonix, m, { args, Health, prefix, command }) => {
    ...
    crypto: async (maureonix, m, { args, Finance }) => {
    ...
    portfolio: async (maureonix, m, { Finance }) => {
    ...
    addstock: async (maureonix, m, { args, Finance, prefix, command }) => {
    ...
    addcrypto: async (maureonix, m, { args, Finance, prefix, command }) => {
    ...
    tip: async (maureonix, m, { args, Finance, prefix, command }) => {
    ...
    loan: async (maureonix, m, { args, Finance, prefix, command }) => {
    ...
    savings: async (maureonix, m, { args, Finance, prefix, command }) => {
    ...
    // Dev
    uuid: async (maureonix, m, { Dev }) => { await m.reply(`🔑 ${Dev.uuid()}`); },
    password: async (maureonix, m, { args, Dev }) => {
        const len = parseInt(args[0]) || 16;
        const p = Dev.password(len);
        await m.reply(`🔐 *Password*\n\`\`\`\n${p.pass}\n\`\`\`\nEntropy: ${p.entropy}`);
    },
    json: async (maureonix, m, { text, Dev, prefix, command }) => {
    ...
    encode: async (maureonix, m, { args, Dev, prefix, command }) => {
    ...
    decode: async (maureonix, m, { args, Dev, prefix, command }) => {
        if (args.length < 2) return m.reply(`Example: ${prefix + command} <base64/url/html> <text>`);
        await m.reply(Dev.decode(args[0], args.slice(1).join(' ')));
    },
    lorem: async (maureonix, m, { args, Dev }) => {
        await m.reply(Dev.lorem(parseInt(args[0]) || 50));
    },
    palette: async (maureonix, m, { Dev }) => {
        const c = Dev.palette();
        await m.reply(`🎨 *Color Palette*\n${c.map(x => `■ ${x}`).join('\n')}`);
    },
    qrvcard: async (maureonix, m, { args, Dev, Tools, maureonix: nm, prefix, command }) => {
    ...
    qrwifi: async (maureonix, m, { args, Dev, Tools, maureonix: nm, prefix, command }) => {
    ...
    checksum: async (maureonix, m, { Dev }) => {
    ...
    convert: async (maureonix, m, { args, prefix, command }) => {
    ...
    // Food
    recipe: async (maureonix, m, { text, Food, prefix, command }) => {
        if (!text) return m.reply(`Example: ${prefix + command} <dish>`);
        const r = await Food.recipe(text);
        if (!r) return m.reply('Recipe not found');
        await maureonix.sendMessage(m.chat, { image: { url: r.thumb }, caption: `🍽️ *${r.name}*\n📍 ${r.area} | ${r.category}\n\n*Ingredients:*\n${r.ingredients.join('\n')}\n\n*Instructions:*\n${r.instructions.slice(0, 800)}...` }, { quoted: m });
    },
    cocktail: async (maureonix, m, { text, Food }) => {
        const c = await Food.cocktail(text || 'margarita');
        if (!c) return m.reply('Drink not found');
        await maureonix.sendMessage(m.chat, { image: { url: c.thumb }, caption: `🍸 *${c.name}*\n🥃 Glass: ${c.glass}\n\n*Ingredients:*\n${c.ingredients.join(', ')}\n\n*How to make:*\n${c.instructions}` }, { quoted: m });
    },
    substitute: async (maureonix, m, { args, Food, prefix, command }) => {
        if (!args[0]) return m.reply(`Example: ${prefix + command} <ingredient>`);
        await m.reply(`🔄 *Substitute for ${args[0]}*\n${Food.substitute(args[0])}`);
    },
    mealprep: async (maureonix, m, { args, Food }) => {
        const plan = Food.mealPrep(args[0] || 'balanced');
        await m.reply(`🥗 *${(args[0] || 'balanced').toUpperCase()} Meal Plan*\n${plan.map((x, i) => `${i + 1}. ${x}`).join('\n')}`);
    },
    // Aliases
    bitcoin: async (maureonix, m, ctx) => { await module.exports.crypto(maureonix, m, { ...ctx, args: ['bitcoin'] }); },
    eth: async (maureonix, m, ctx) => { await module.exports.crypto(maureonix, m, { ...ctx, args: ['ethereum'] }); },
    emi: async (maureonix, m, ctx) => { await module.exports.loan(maureonix, m, ctx); },
    tags: async (maureonix, m, ctx) => { await module.exports.hashtag(maureonix, m, ctx); },
    phrases: async (maureonix, m, ctx) => { await module.exports.phrasebook(maureonix, m, ctx); },
    time: async (maureonix, m, ctx) => { await module.exports.worldclock(maureonix, m, ctx); },
    unit: async (maureonix, m, ctx) => { await module.exports.convert(maureonix, m, ctx); },
};