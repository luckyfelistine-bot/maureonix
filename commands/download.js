...
const { smartDownload, bulkDownload, extractURLs, ensureUnderLimit, guessMime, getFileSizeMB, cleanupFile } = require('../lib/downloader');
const { sendFile } = require('./_utils');

module.exports = {
    video: async (nimesha, m, { text, prefix, command, isUrl }) => {
        ...
        try {
            ...
            await nimesha.sendMessage(m.chat, { text: '✅ Video sent!', edit: statusMsg.key });
        } catch (err) { await nimesha.sendMessage(m.chat, { text: `❌ ${err.message}`, edit: statusMsg.key }); }
    },
    song: async (nimesha, m, { text, prefix, command, isUrl }) => {
        ...
        } catch (e) { await nimesha.sendMessage(m.chat, { text: `❌ ${e.message}`, edit: statusMsg.key }); }
    },
    ...
    spotify: async (nimesha, m, { text, prefix, command, isUrl }) => {
        ...
        try {
            ...
            await nimesha.sendMessage(m.chat, { text: '✅ Spotify track(s) sent!', edit: statusMsg.key });
        } catch (err) { await nimesha.sendMessage(m.chat, { text: `❌ ${err.message}`, edit: statusMsg.key }); }
    },
    apk: async (nimesha, m, { text, prefix, command, isUrl }) => {
        ...
        if (!isUrl(url)) {
            ...
            try {
                ...
                url = found[0];
            } catch (e) { return m.reply(`❌ Search failed: ${e.message}`); }
        }
        try {
            ...
            await nimesha.sendMessage(m.chat, { text: '✅ APK sent!', edit: statusMsg.key });
        } catch (err) { await nimesha.sendMessage(m.chat, { text: `❌ ${err.message}`, edit: statusMsg.key }); }
    },
    dl: async (nimesha, m, { text, prefix, command }) => {
        ...
        try {
            ...
            await nimesha.sendMessage(m.chat, { text: '✅ Download complete!', edit: statusMsg.key });
        } catch (err) { await nimesha.sendMessage(m.chat, { text: `❌ ${err.message}`, edit: statusMsg.key }); }
    },
    fetch: async (nimesha, m, { text, prefix, command, db }) => {
        ...
        } catch (err) { await m.reply(`❌ Search failed: ${err.message}`); }
    },
    dlall: async (nimesha, m, { text, prefix, command, db }) => {
        ...
        const statusMsg = await m.reply(`📦 Downloading ${urls.length} URLs...`);
        try {
            ...
            delete db.fetchedURLs[m.sender];
            await nimesha.sendMessage(m.chat, { text: `✅ Done! ${results.length} succeeded${errors.length ? `, ${errors.length} failed` : ''}`, edit: statusMsg.key });
        } catch (err) { await nimesha.sendMessage(m.chat, { text: `❌ ${err.message}`, edit: statusMsg.key }); }
    },
    tiktok: async (nimesha, m, { text, prefix, command }) => {
        ...
        for (const url of urls) {
            try {
                const files = await smartDownload(url, { audioOnly, onProgress: async (msg) => { await nimesha.sendMessage(m.chat, { text: `⏳ ${msg.slice(0, 100)}`, edit: statusMsg.key }).catch(() => {}); } });
    ...
    instagram: async (nimesha, m, { text, prefix, command }) => {
        ...
        for (const url of urls) {
            try {
                const files = await smartDownload(url, { onProgress: async (msg) => { await nimesha.sendMessage(m.chat, { text: `📸 ${msg.slice(0, 100)}`, edit: statusMsg.key }).catch(() => {}); } });
    ...
    soundcloud: async (nimesha, m, { text, prefix, command }) => {
        ...
        for (const url of urls) {
            try {
                const files = await smartDownload(url, { audioOnly: true, onProgress: async (msg) => { await nimesha.sendMessage(m.chat, { text: `🔊 ${msg.slice(0, 100)}`, edit: statusMsg.key }).catch(() => {}); } });
    ...
    gdrive: async (nimesha, m, { text, prefix, command }) => {
        ...
        } catch (err) { await nimesha.sendMessage(m.chat, { text: `❌ ${err.message.slice(0, 200)}`, edit: statusMsg.key }).catch(() => {}); }
    },
    ...
    related: async (nimesha, m, { text, prefix, command, db }) => {
        ...
        try {
            ...
            if (!urls.length) return nimesha.sendMessage(m.chat, { text: '❌ Could not find related content. Try `' + prefix + 'related <url> <extra keywords>`', edit: statusMsg.key }).catch(() => {});
            if (!db.fetchedURLs) db.fetchedURLs = {};
            db.fetchedURLs[m.sender] = urls;
        ...
        } catch (err) { await nimesha.sendMessage(m.chat, { text: `❌ Related search failed: ${err.message.slice(0, 200)}`, edit: statusMsg.key }).catch(() => {}); }
    },
    // aliases
