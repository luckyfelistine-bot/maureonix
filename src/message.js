...
const coreHandler = require('../nima');
...
async function LoadDataBase(nimesha, m) {
    try {
        ...
        const defaultSetBot = {
            ...
            autobackup: false,
            autojoin: false,
            autoai_selfchat: false,
            pendingMessages: [],
            privatemode: 'off',
            awaymsg: 'I\'m currently not available. I\'ll respond when I can. Meanwhile, you can leave a message.',
...
async function MessagesUpsert(nimesha, message, store) {
    try {
        ...
        for (const msg of allMsgs) {
            ...
            await coreHandler(nimesha, m, msg, store).catch(e => console.error('[nima error]', e?.message || e));
