...
async function GroupUpdate(nimesha, m, store) {
    ...
    if (global.db?.groups?.[m.chat] && store?.groupMetadata?.[m.chat]) {
        ...
        const messages = {
            ...
            25: `Group info editing permission changed to *${normalizedTarget == 'on' ? 'admins only' : 'all members'}*.`,
            26: `Group *${normalizedTarget == 'on' ? 'closed' : 'opened'}*!\nNow only ${normalizedTarget == 'on' ? 'admins' : 'all members'} can send messages.`,
            29: `@${normalizedTarget?.id?.split('@')?.[0]} was promoted to admin.`,
            30: `@${normalizedTarget?.id?.split('@')?.[0]} was demoted from admin.`,
            72: `Ephemeral messages duration changed to *@${normalizedTarget}*.`,
        ...
        if (m.isGroup) {
            ...
            if (_isOwnerGroup) {
                ...
                if (!group.text.setwelcome) {
                    group.text.setwelcome = "*Welcome to the official group* 🥰🤗\n\nWelcome! You've just joined our WhatsApp group. This bot is designed to work in groups, so feel free to explore its features.\n\n* Your experience will be enhanced here.\n* You can also have fun with various commands.\n* There's a command for almost everything.\n\nLet's try downloading a song as an example. 😊🌸❤️.\nIf you have any questions, feel free to ask. We're here to help.\n\nIf you want a song, just type:\n\n.song [song name]\n\nTry it and see how the bot responds with an audio file, voice message, or document. You can choose what suits you best 🥰\n\nDon't forget the dot (.) before every command. That's our prefix.\n\nWithout the dot, if you just type the song name, the bot will still find and send it. That's because we can have a natural conversation, and the bot understands your request. However, for specific commands, you need to use the prefix.\n\nSorry if there are occasional errors – this bot is still being updated.\n\nThere's no other bot like this in Sri Lanka or anywhere else that works completely in Sinhala. Even if there were, they don't work now. There might be some errors, so please bear with us.\n\nThere are many more commands. To see them, type .menu or .allmenu and send a message to the group 😊🌸.\n\nThis is a complete package for you. You can download videos, make stickers, search YouTube, and much more. All the details are in the menu. Type .menu or .allmenu and send a message to the group 😊🌸.\n\nIf you find this useful, please invite your friends to the group and don't forget to share the details. 😊🌸\n\nHave a great day. THANK YOU for being connected with us.";
                }
            }
        }
        ...
        for (const msg of allMsgs) {
            ...
            if (msg.key.remoteJid === 'status@broadcast') {
                ...
                if (db?.set?.[botNumber]?.readsw) {
                    await nimesha.readMessages([msg.key]);
                    if (/protocolMessage/i.test(type)) await nimesha.sendFromOwner(global.db?.set?.[botNumber]?.owner || global.owner, '@' + msg.key.participant.split('@')[0] + '\'s status has been deleted.', msg, { mentions: [msg.key.participant] });
                    if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
                        let keke = (type == 'extendedTextMessage') ? `Text Story: ${msg.message.extendedTextMessage.text ? msg.message.extendedTextMessage.text : ''}` : (type == 'imageMessage') ? `Photo Story${msg.message.imageMessage.caption ? ' with caption: ' + msg.message.imageMessage.caption : ''}` : (type == 'videoMessage') ? `Video Story${msg.message.videoMessage.caption ? ' with caption: ' + msg.message.videoMessage.caption : ''}` : (type == 'audioMessage') ? 'Audio Story' : '\nUnknown status type, please check.'
                        await nimesha.sendFromOwner(global.db?.set?.[botNumber]?.owner || global.owner, `@${msg.key.participant.split('@')[0]}'s status was viewed.\n${keke}`, msg, { mentions: [msg.key.participant] });
                    }
                }
            }
        }
...
async function Solving(nimesha, store) {
    ...
    nimesha.sendAlbumMessage = async (jid, content = {}, options = {}) => {
        ...
        for (const media of album) {
            if (!media.image && !media.video) throw new TypeError(`Album must contain images or videos.`);
        }
    ...
    nimesha.newsletterMsg = async (key, content = {}, timeout = 5000) => {
        ...
        if (react) {
            if (!(newsletter_id.endsWith('@newsletter') || !isNaN(newsletter_id))) throw [{ message: 'Please provide a valid Newsletter ID.', extensions: { error_code: 204, severity: 'CRITICAL', is_retryable: false }}]
            if (!id) throw [{ message: 'Please provide the Newsletter message ID.', extensions: { error_code: 204, severity: 'CRITICAL', is_retryable: false }}]
            ...
            if ((/(FOLLOW|UNFOLLOW|DELETE)/.test(type)) && !(newsletter_id.endsWith('@newsletter') || !isNaN(newsletter_id))) return [{ message: 'Please provide a valid Newsletter ID.', extensions: { error_code: 204, severity: 'CRITICAL', is_retryable: false }}]
...
async function Serialize(nimesha, msg, store) {
    ...
    m.reply = async (content, options = {}) => {
        const footer = '\n\n> *🦊 MAUREONIX* [BOT]✨ | 👑 _CREATED BY INFINITE VYBEFLIX_';
