...
function checkChips(){var hasText=false;var inputs=['chatInput','neuralInput'];inputs.forEach(function(id){var el=document.getElementById(id);if(el&&el.value.trim().length>0)hasText=true;});var chips=document.querySelectorAll('.neural-chips');chips.forEach(function(c){if(hasText)c.classList.add('hidden');else c.classList.remove('hidden');});}
function sendQuick(text){var ta=currentTab==='neural'?document.getElementById('neuralInput'):document.getElementById('chatInput');ta.value=text;autoResize(ta);checkChips();sendChatMessage();}
async function sendChatMessage(){var ta=currentTab==='neural'?document.getElementById('neuralInput'):document.getElementById('chatInput');var btn=currentTab==='neural'?document.getElementById('neuralSend'):document.getElementById('chatSend');var text=ta.value.trim();if(!text)return;ta.value='';autoResize(ta);checkChips();appendChatBubble('user',text,Date.now(),true);var typingEl=currentTab==='neural'?document.getElementById('neuralTyping'):document.getElementById('chatTyping');if(typingEl)typingEl.style.display='flex';btn.disabled=true;try{var res=await fetch('/api/ai/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sessionId:chatSessionId,message:text})});var data=await res.json();if(data.text){appendChatBubble('assistant',data.text,Date.now(),true);if(data.actions&&Array.isArray(data.actions)){data.actions.forEach(function(act){if(act.type==='scrollTo'){var t=document.getElementById(act.target);if(t)t.scrollIntoView({behavior:'smooth',block:'start'});}});}}else{appendChatBubble('assistant','⚠️ Neural link interrupted. Please retry.',Date.now(),true);}}catch(e){appendChatBubble('assistant','❌ Connection failed. The neural network is unreachable.',Date.now(),true);}if(typingEl)typingEl.style.display='none';btn.disabled=false;ta.focus();}
