...
function initChatSession(){try{chatSessionId=localStorage.getItem(CHAT_SESSION_KEY)||generateSessionId();localStorage.setItem(CHAT_SESSION_KEY,chatSessionId);var raw=localStorage.getItem(CHAT_HISTORY_KEY);chatHistory=raw?JSON.parse(raw):[];renderChatHistory();if(!chatHistory.length){appendChatBubble('assistant','Welcome to the **Neural Command Center**.\\n\\nI am Maureonix. Ask me about commands, pairing, or anything.',Date.now(),true);}}catch(e){chatSessionId=generateSessionId();}}
function saveChatHistory(){try{localStorage.setItem(CHAT_HISTORY_KEY,JSON.stringify(chatHistory.slice(-50)));}catch(e){}}
function renderChatHistory(){var container=currentTab==='neural'?document.getElementById('neuralMessages'):document.getElementById('chatMessages');if(!container)return;container.innerHTML='';chatHistory.forEach(function(msg){appendChatBubble(msg.role,msg.text,msg.ts,false);});scrollChatToBottom();}
...
function copyCode(btn){var code=btn.parentElement.querySelector('code').innerText;navigator.clipboard.writeText(code).then(function(){btn.textContent='COPIED';setTimeout(function(){btn.textContent='COPY';},2000);});}
function appendChatBubble(role,text,ts,save){var container=currentTab==='neural'?document.getElementById('neuralMessages'):document.getElementById('chatMessages');if(!container)return;var bubble=document.createElement('div');bubble.className='chat-bubble '+role;var timeStr=ts?new Date(ts).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'';var parsed=role==='assistant'?parseMarkdown(text):text.replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\\n/g,'<br>');bubble.innerHTML=parsed+'<span class="ts">'+timeStr+'</span>';container.appendChild(bubble);if(save){chatHistory.push({role,text,ts:ts||Date.now()});saveChatHistory();}scrollChatToBottom();}
function scrollChatToBottom(){var container=currentTab==='neural'?document.getElementById('neuralMessages'):document.getElementById('chatMessages');if(container)container.scrollTop=container.scrollHeight;}
...
function sendQuick(text){var ta=currentTab==='neural'?document.getElementById('neuralInput'):document.getElementById('chatInput');ta.value=text;autoResize(ta);sendChatMessage();}
async function sendChatMessage(){var ta=currentTab==='neural'?document.getElementById('neuralInput'):document.getElementById('chatInput');var btn=currentTab==='neural'?document.getElementById('neuralSend'):document.getElementById('chatSend');var text=ta.value.trim();if(!text)return;ta.value='';autoResize(ta);appendChatBubble('user',text,Date.now(),true);var typingEl=currentTab==='neural'?document.getElementById('neuralTyping'):document.getElementById('chatTyping');if(typingEl)typingEl.style.display='flex';btn.disabled=true;try{var res=await fetch('/api/ai/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sessionId:chatSessionId,message:text})});var data=await res.json();if(data.text){appendChatBubble('assistant',data.text,Date.now(),true);if(data.actions&&Array.isArray(data.actions)){data.actions.forEach(function(act){if(act.type==='scrollTo'){var t=document.getElementById(act.target);if(t)t.scrollIntoView({behavior:'smooth',block:'start'});}});}}else{appendChatBubble('assistant','⚠️ Neural link interrupted. Please retry.',Date.now(),true);}}catch(e){appendChatBubble('assistant','❌ Connection failed. The neural network is unreachable.',Date.now(),true);}if(typingEl)typingEl.style.display='none';btn.disabled=false;ta.focus();}
function toggleChat(){var panel=document.getElementById('chatPanel');var btn=document.getElementById('chatToggle');isChatOpen=!isChatOpen;panel.style.display=isChatOpen?'flex':'none';if(isChatOpen){document.getElementById('chatInput').focus();scrollChatToBottom();}}
document.addEventListener('keydown',function(e){if(e.key==='Escape')closeModal();if(e.key==='Enter'&&document.activeElement.id==='secretInput')doLogin();if(e.key>='1'&&e.key<='4'&&document.activeElement.tagName!=='TEXTAREA'&&document.activeElement.tagName!=='INPUT'){var tabs=['dashboard','feedback','visitors','neural'];switchTab(tabs[parseInt(e.key)-1]);}});
